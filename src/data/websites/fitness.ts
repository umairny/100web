import { imageUrl } from "../../assets/optimized";
import type { WebsiteDesign } from "../websites";
export const fitnessWebsites: WebsiteDesign[] = [
  {
    id: "pulseforge-fitness",
    title: "PulseForge Fitness",
    category: "Fitness",
    style: "high-energy, disciplined, modern, performance-focused",
    shortDescription:
      "Strength and conditioning gym with coaching, class schedules, transformation proof, and trial membership flow",
    slug: "pulseforge-fitness",
    image: imageUrl("fitness/pulseforge-fitness.webp"),
    colors: {
      primary: "#14532d",
      secondary: "#ecfdf5",
      accent: "#f97316",
      dark: "#111827",
    },
    status: "completed",
  },
  {
    id: "corelab-pilates",
    title: "CoreLab Pilates",
    category: "Fitness",
    style: "calm, refined, breathable, wellness-focused",
    shortDescription:
      "Peaceful mat, reformer, mobility, and core-strength classes with thoughtful guidance and a welcoming studio atmosphere",
    slug: "corelab-pilates",
    image: imageUrl("fitness/corelab-pilates.webp"),
    colors: {
      primary: "#B56F59",
      secondary: "#F7F0E7",
      accent: "#6F7D61",
      dark: "#373330",
    },
    status: "completed",
  },
  {
    id: "irondistrict-gym",
    title: "IronDistrict Gym",
    category: "Fitness",
    style: "hardcore, industrial, disciplined, performance-focused",
    shortDescription:
      "A serious strength facility with heavy equipment, disciplined programming, and focused coaching for lifters who train with purpose",
    slug: "irondistrict-gym",
    image: imageUrl("fitness/irondistrict-gym.webp"),
    colors: {
      primary: "#111318",
      secondary: "#C9CDD1",
      accent: "#F6C945",
      dark: "#0A0B0D",
    },
    status: "completed",
  },
  {
    id: "peakrun-coaching",
    title: "PeakRun Coaching",
    category: "Fitness",
    style: "clean, energetic, outdoorsy, performance-focused",
    shortDescription:
      "Running coaching website with structured plans, race preparation, pacing support, progress tracking, and membership packages",
    slug: "peakrun-coaching",
    image: imageUrl("fitness/peakrun-coaching.webp"),
    colors: {
      primary: "#0B2A4A",
      secondary: "#F5F8FB",
      accent: "#17A7E8",
      dark: "#071B33",
    },
    status: "completed",
  },
  {
    id: "flowstate-yoga",
    title: "FlowState Yoga",
    category: "Fitness",
    style: "calm, premium, organic, mindful, restorative",
    shortDescription:
      "Peaceful yoga studio website for mindful movement, breathwork, meditation, restorative classes, teachers, and memberships",
    slug: "flowstate-yoga",
    image: imageUrl("fitness/flowstate-yoga.webp"),
    colors: {
      primary: "#5F6F58",
      secondary: "#F8F2EA",
      accent: "#B97358",
      dark: "#332F2A",
    },
    status: "completed",
  },
  {
    id: "boxhouse-training",
    title: "BoxHouse Training",
    category: "Fitness",
    style: "bold, gritty, athletic, disciplined, high-energy",
    shortDescription:
      "Boxing and conditioning studio with technique classes, heavy bag rounds, strength circuits, coaching, and memberships",
    slug: "boxhouse-training",
    image: imageUrl("fitness/boxhouse-training.webp"),
    colors: {
      primary: "#0A0B0C",
      secondary: "#F8EFE2",
      accent: "#E24835",
      dark: "#101112",
    },
    status: "completed",
  },
  {
    id: "vitalform-wellness",
    title: "VitalForm Wellness",
    category: "Fitness",
    style: "bright, health-tech, editorial, balanced, wellness-focused",
    shortDescription:
      "Integrated wellness platform combining fitness coaching, nutrition guidance, recovery support, dashboard metrics, weekly planning, and lifestyle-focused coaching plans",
    slug: "vitalform-wellness",
    image: imageUrl("fitness/vitaform-wellness.webp"),
    colors: {
      primary: "#174136",
      secondary: "#FBFAF5",
      accent: "#5ECFB6",
      dark: "#53635F",
    },
    status: "completed",
  },
  {
    id: "ridehaus-cycling",
    title: "RideHaus Cycling",
    category: "Fitness",
    style: "neon, music-driven, immersive, high-energy, event-inspired",
    shortDescription:
      "Premium indoor cycling studio with rhythm-based rides, dramatic lighting, coach-led classes, weekly schedule, performance metrics, community moments, and membership paths",
    slug: "ridehaus-cycling",
    image: imageUrl("fitness/redehaus-cycling.webp"),
    colors: {
      primary: "#070817",
      secondary: "#20E7FF",
      accent: "#FF2FCB",
      dark: "#0B0D20",
    },
    status: "completed",
  },
  {
    id: "elevate-climbing",
    title: "Elevate Climbing",
    category: "Fitness",
    style:
      "chalky, playful, route-map inspired, adventurous, community-focused",
    shortDescription:
      "Modern bouldering gym website with climbing zones, skill classes, weekly route setting, community events, route-team coaches, memberships, and progress visuals",
    slug: "elevate-climbing",
    image: imageUrl("fitness/elevate-climbing.webp"),
    colors: {
      primary: "#25303A",
      secondary: "#F7F1E6",
      accent: "#21A6A1",
      dark: "#606A70",
    },
    status: "completed",
  },
  {
    id: "reset-recovery-club",
    title: "Reset Recovery Club",
    category: "Fitness",
    style: "serene, spa-tech, frosted-glass, clinical-editorial, restorative",
    shortDescription:
      "Premium recovery studio website with mobility sessions, assisted stretching, compression lounge, reset method, recovery tech, memberships, journal content, and calm testimonials",
    slug: "reset-recovery-club",
    image: imageUrl("fitness/resetrecovery-club.webp"),
    colors: {
      primary: "#23313B",
      secondary: "#F6F8F7",
      accent: "#75B7A6",
      dark: "#607075",
    },
    status: "completed",
  },
];
