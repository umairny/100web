...
const principles = [
  [
    "01",
    "Distinct by design",
    "Every concept has its own visual language, audience, and conversion rhythm.",
  ],
  [
    "02",
    "Built to be felt",
    "Typography, photography, color, and motion work together to create a memorable atmosphere.",
  ],
  [
    "03",
    "Ready to convert",
    "Clear services, social proof, and thoughtful booking paths turn browsing into confident action.",
  ],
];
    ...
    <main className="beauty-index bg-[#f5f0eb] text-[#181514]">
        ...
        <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1480px] flex-col px-5 pb-8 pt-24 sm:px-8 lg:px-12 lg:pb-10 lg:pt-28">
          ...
          <div className="grid flex-1 items-center py-14 lg:grid-cols-[minmax(0,760px)_1fr]">
            ...
            >
              ...
              <p className="mt-4 max-w-xl text-sm font-semibold capitalize tracking-wide text-[#e9c0b2]">
                {featured.style}
              </p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Link
                  to={`/beauty/${featured.slug}`}
                  className="group inline-flex min-h-14 items-center justify-between gap-10 bg-white px-6 text-sm font-black uppercase tracking-[.1em] text-[#181514] transition hover:bg-[#e9c0b2]"
                >
                  Explore Website{" "}
                ...
                <a
                  href="#collection"
                  className="inline-flex min-h-14 items-center justify-center border border-white/35 px-6 text-sm font-bold uppercase tracking-[.1em] text-white transition hover:bg-white/10"
                >
                  View Collection
                </a>
              </div>
          ...
          <div className="grid gap-6 border-t border-white/20 pt-5 lg:grid-cols-[1fr_auto] lg:items-end">
                  ...
                  <span className="block h-px overflow-hidden bg-white/25">
                    {activeSlide === index && (
                      <span
                        ...
                        className={`beauty-progress block h-full bg-[#efb49f] ${paused ? "[animation-play-state:paused]" : ""}`}
                      />
                    )}
                  </span>
                  <span className="mt-3 hidden truncate text-[.62rem] font-bold uppercase tracking-[.1em] xl:block">
                    {website.title}
                  </span>
                </button>
              ))}
            ...
            <div className="flex gap-2 justify-self-end">
              <button
                type="button"
                onClick={() => moveSlide(-1)}
                className="grid h-12 w-12 place-items-center border border-white/30 text-xl transition hover:bg-white hover:text-black"
      ...
      <section className="border-b border-[#d9d0c8] bg-[#efe7e0]">
        <div className="mx-auto grid max-w-[1480px] grid-cols-2 divide-x divide-[#d9d0c8] px-5 sm:grid-cols-4 sm:px-8 lg:px-12">
          {[
            [websites.length, "Live websites"],
            ["10", "Unique identities"],
            ["100%", "Responsive"],
            ["∞", "Ways to glow"],
          ].map(([value, label]) => (
            <div key={label} className="px-4 py-7 text-center">
              <b className="beauty-display block text-3xl font-normal">
                {value}
              </b>
              <span className="mt-1 block text-[.68rem] font-bold uppercase tracking-[.14em] text-[#756b66]">
                {label}
              </span>
            </div>
          ))}
      ...
      <section id="collection" className="px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-[1480px]">
          <div className="grid gap-8 border-b border-[#cfc4bc] pb-10 lg:grid-cols-[1fr_1fr] lg:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[.22em] text-[#a25f58]">
                The live collection
              </p>
              <h2 className="beauty-display mt-4 max-w-3xl text-5xl leading-[.96] tracking-[-.045em] sm:text-6xl lg:text-7xl">
                Ten brands. Ten completely different moods.
              </h2>
            </div>
            <p className="max-w-2xl text-base leading-8 text-[#665e59] lg:justify-self-end">
              Explore finished, responsive beauty websites spanning editorial
              hair, wellness, skincare, bridal artistry, nails, makeup, and
              modern grooming.
            </p>
          </div>

          <div
            className="flex gap-2 overflow-x-auto py-7 [scrollbar-width:none]"
          ...
          >
            {filters.map((item) => (
              <button
                ...
                className={`shrink-0 rounded-full border px-5 py-2.5 text-sm font-bold transition ${filter === item ? "border-[#181514] bg-[#181514] text-white" : "border-[#cfc4bc] bg-transparent text-[#665e59] hover:border-[#181514] hover:text-[#181514]"}`}
              >
                {item}
              </button>
            ))}
      ...
      <section className="bg-[#1b1817] px-5 py-20 text-white sm:px-8 lg:px-12 lg:py-28">
        <div className="mx-auto max-w-[1480px]">
          <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
            <div>
              <p className="text-xs font-black uppercase tracking-[.22em] text-[#eeb49f]">
                Design philosophy
              </p>
              <h2 className="beauty-display mt-4 text-5xl leading-none tracking-[-.045em] sm:text-6xl">
                Beautiful is only the beginning.
              </h2>
            </div>
            <p className="max-w-xl self-end text-base leading-8 text-white/60 lg:justify-self-end">
              Each concept is designed to earn attention, build trust, and make
          ...
          <div className="mt-14 grid border-y border-white/15 md:grid-cols-3 md:divide-x md:divide-white/15">
            {principles.map(([number, title, text]) => (
              <article
                key={number}
                className="border-b border-white/15 px-2 py-9 last:border-b-0 md:border-b-0 md:px-8 md:py-12"
              >
                <span className="text-xs font-bold text-[#eeb49f]">
                  {number}
                </span>
                <h3 className="beauty-display mt-8 text-3xl">{title}</h3>
      ...
      <section className="relative overflow-hidden bg-[#e8b4a2] px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
        <div className="absolute -right-24 -top-36 h-96 w-96 rounded-full border border-[#181514]/15" />
        <div className="absolute -right-5 -top-16 h-64 w-64 rounded-full border border-[#181514]/15" />
        <div className="relative mx-auto flex max-w-[1480px] flex-col justify-between gap-8 lg:flex-row lg:items-end">
          <div>
            ...
            <h2 className="beauty-display mt-4 max-w-4xl text-5xl leading-[.95] tracking-[-.05em] sm:text-7xl">
              Find the beauty brand that feels like you.
            </h2>
          </div>
          <a
            href="#collection"
            className="group inline-flex min-h-14 shrink-0 items-center justify-between gap-12 bg-[#181514] px-7 text-sm font-black uppercase tracking-[.1em] text-white"
          >
            Browse all websites{" "}
