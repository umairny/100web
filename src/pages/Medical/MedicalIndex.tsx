...
export function MedicalIndex() {
  ...
  const plannedRoadmapCount = Math.max(
  ...
  return (
    <main className="bg-[#fff8f2] text-[#18313a]">
        ...
        <Container>
          <AnimatedSection className="grid min-h-[calc(100vh-5rem)] gap-12 py-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:py-16">
            <div>
              <Link
                to="/"
                className="inline-flex text-sm font-bold text-[#6e6f55] transition hover:text-[#18313a]"
              >
                Back to Home
              </Link>
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-[#5d6d66] md:text-xl">
                A healthcare design hub focused on trust, service clarity,
                appointment confidence, and friendly paths into care.
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <CTAButton
                  href="#live-concepts"
                  size="lg"
                  className="rounded-full bg-[#18313a] text-white hover:bg-[#8a4d5c]"
                >
                  View Live Concepts
                </CTAButton>
                <CTAButton
              ...
              <div className="grid gap-4 rounded-[3rem] border border-white bg-white/74 p-4 shadow-2xl shadow-[#8a4d5c]/14 backdrop-blur sm:grid-cols-[0.92fr_1.08fr] sm:items-end">
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-[2.25rem] rounded-bl-[5rem] bg-[#18313a] p-6 text-white shadow-lg shadow-[#18313a]/16">
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-[#f3c8bd]">
                      Medical UX focus
                    </p>
                    <p className="mt-3 font-serif text-3xl leading-tight">
                      Book clearly, understand care, follow the plan.
                    </p>
                  </div>
                  <img
        ...
        <Container>
          <div className="flex flex-wrap justify-center gap-2">
            {careTracks.map((track) => (
              <span
                key={track}
                className="rounded-full border border-[#efd5cc] bg-[#fff8f2] px-5 py-2 text-sm font-bold text-[#6c6158]"
              >
                {track}
              </span>
            ))}
      ...
      <section
        id="live-concepts"
        className="relative isolate overflow-hidden bg-[#18313a] py-20 text-white md:py-28"
      >
        <svg
          aria-hidden="true"
        ...
        <Container>
          <AnimatedSection className="mb-10 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-[#f3c8bd]">
                Live concepts
              </p>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={label}
                  className="rounded-[2rem] rounded-br-[4rem] bg-white/10 p-4 text-center ring-1 ring-white/14"
                >
                  <p className="font-serif text-4xl text-[#f3c8bd]">{value}</p>
                  <p className="mt-1 text-xs font-black uppercase text-white/60">
                    {label}
                  </p>
                </div>
              ))}
          ...
          <div className="mt-5 rounded-[2rem] border border-white/12 bg-white/8 p-5">
            <p className="text-sm leading-7 text-white/68">
              {liveWebsites.length} healthcare demos are live now, each with a
              different specialty, visual system, and appointment path.{" "}
              {plannedRoadmapCount} concepts remain in the medical roadmap.
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {uxNotes.map(({ title, text, icon: Icon }) => (
              <article
                key={title}
                className="rounded-[2rem] rounded-tl-[4rem] bg-white/10 p-6 ring-1 ring-white/14"
              >
      ...
      <section id="design-system" className="bg-[#fff8f2] py-20 md:py-28">
        <Container>
          <AnimatedSection className="mb-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-[#8a4d5c]">
                Visual system
              </p>
              <h2 className="mt-4 font-serif text-4xl leading-tight text-[#18313a] md:text-5xl">
                Healthcare pages work better when patients can picture the
                experience.
              </h2>
            </div>
            <p className="max-w-xl text-lg leading-8 text-[#5d6d66]">
              The index now uses softer curves, image-led sections, and calmer
              labels so the collection feels curated instead of crowded.
            </p>
          </AnimatedSection>
          <div className="grid gap-5 md:grid-cols-3">
            {gallery.map(({ title, text, image }) => (
              <article
                key={title}
                className="overflow-hidden rounded-[2.5rem] rounded-tr-[6rem] bg-white shadow-sm shadow-[#14313f]/6 ring-1 ring-[#efd5cc]"
              >
                <img
                  src={imageUrl(image)}
                  alt=""
                ...
                />
                <div className="p-6">
                  <h3 className="font-serif text-3xl text-[#18313a]">
                    {title}
                  </h3>
      ...
      <section
        id="roadmap"
        className="border-y border-[#efd5cc] bg-white py-20 md:py-28"
      >
        <Container>
          <AnimatedSection className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              ...
              <p className="mt-5 max-w-xl text-lg leading-8 text-[#5d6d66]">
                All 10 healthcare concepts are now live, each with its own
                specialty, visual system, calls to action, and patient-first
            ...
            <div className="rounded-[3rem] rounded-bl-[7rem] border border-[#efd5cc] bg-[#fff8f2] p-8 shadow-sm shadow-[#14313f]/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-[#8a4d5c]">
                    Live concepts
                  </p>
                </div>
              </div>
              <p className="mt-6 text-sm leading-7 text-[#5d6d66]">
                Browse the live concept board above to compare primary care,
                pediatrics, dental, optometry, physical therapy, counseling,
                urgent care, women's health, cardiology, and dermatology
                designs.
              </p>
              <div className="mt-6 flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={item}
                    className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#6c6158] ring-1 ring-[#efd5cc]"
                  >
                    {item}
                  </span>
                ))}
