...
const trustPoints = ['Clear intake process', 'In-person and virtual options', 'Supportive therapist guidance']

const heroSlides = [
  {
    label: 'A calm first conversation',
    text: 'Comfortable therapy spaces for private, human-centered intake.',
    image: 'medical/mindwell/hero.png',
  ...
  },
  {
    ...
    image: 'medical/mindwell/calm-office.png',
  ...
  },
  {
    ...
    image: 'medical/mindwell/therapy-session.png',
...
const visitDetails = [
  ['What to expect', 'A warm intake conversation, practical questions, and clear explanation of possible next steps.'],
  ['First appointment', 'Your therapist will learn about your concerns, preferences, goals, and comfort with care.'],
  ['In-person visits', 'Appointments take place in a calm office setting designed for private conversation.'],
  ['Virtual visits', 'Virtual appointments can support ongoing sessions and busy schedules.'],
  ['Payment and plan questions', 'Contact the practice to review payment and plan details before your visit.'],
  ['Ongoing sessions', 'Follow-up timing depends on your needs, provider fit, and the care path you choose together.'],
]
...
const reviews = [
  {
    name: 'Maya',
    quote: 'The intake felt simple, and I understood what the next step would be before the first session.',
  },
  {
    name: 'Elliot',
    quote: 'I appreciated that the first conversation was clear, respectful, and easy to follow.',
  },
  {
    name: 'Nina',
    quote: 'They helped me understand appointment options without making the process feel overwhelming.',
  },
]

const FooterLink = ({ label, href }: { label: string; href: string }) => (
  <a href={href} className="text-sm font-semibold text-white/70 transition hover:text-white">
    {label}
  </a>
)
...
export function MindWellCounseling() {
  ...
  return (
    <main className="bg-[#f8f3ea] text-[#1f2933]">
          ...
          <div className="flex flex-col justify-center rounded-[2rem] border border-[#e4d9c9] bg-[#fffaf1]/82 p-6 shadow-2xl shadow-[#2d4b58]/8 backdrop-blur md:p-9">
            <div className="flex flex-wrap items-center gap-3">
              <p className="inline-flex items-center gap-2 rounded-full border border-[#decfb9] bg-white/70 px-4 py-2 text-sm font-bold text-[#657d68]">
                <ShieldCheck aria-hidden="true" size={17} /> Approachable therapy intake
              </p>
              <a href="#teletherapy" className="inline-flex items-center gap-2 rounded-full bg-[#eef3ec] px-4 py-2 text-sm font-bold text-[#2d4b58] transition hover:bg-[#dfe9e5]">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#5e635f] md:text-xl">
              MindWell Counseling helps clients begin therapy with a clear intake process, thoughtful provider guidance,
              and practical next steps for care that feels private, approachable, and human.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <CTAButton href="#contact" size="lg" className="rounded-full bg-[#2d4b58] text-white hover:bg-[#203842]">
                Start Intake
              </CTAButton>
              <CTAButton
                href="#services"
            ...
            <div className="mt-8 grid gap-3 border-t border-[#e4d9c9] pt-6 sm:grid-cols-3">
              {trustPoints.map((point) => (
                <div key={point} className="text-sm font-bold text-[#4f5f59]">
                  <CheckCircle aria-hidden="true" className="mb-2 text-[#657d68]" size={18} />
                  {point}
                </div>
              ))}
            </div>
          ...
          <div className="relative grid gap-4">
            <div className="relative min-h-[34rem] overflow-hidden rounded-[2rem] bg-[#18242c] shadow-2xl shadow-[#2d4b58]/16 md:min-h-[42rem]">
              ...
              <div className="absolute bottom-5 left-5 right-5 rounded-[1.5rem] border border-white/30 bg-[#fffaf1]/90 p-5 text-[#18242c] shadow-xl shadow-[#18242c]/14 backdrop-blur md:bottom-6 md:right-auto md:w-[25rem]">
                <p className="text-xs font-black uppercase text-[#9b7d42]">{activeSlide.label}</p>
                <p className="mt-2 text-lg font-bold leading-7">{activeSlide.text}</p>
              </div>
                ...
                {heroSlides.map((slide, index) => (
                  <button
                    ...
                    className={`h-2.5 rounded-full transition ${
                      index === activeHeroSlide ? 'w-8 bg-white' : 'w-2.5 bg-white/52 hover:bg-white/78'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
              <div className="rounded-[1.5rem] border border-[#e4d9c9] bg-[#fffaf1] p-5 shadow-xl shadow-[#2d4b58]/8">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase text-[#9b7d42]">Intake steps</p>
                    <p className="mt-1 text-lg font-black text-[#18242c]">A clear path before the first visit.</p>
                  </div>
                  <MessageCircle aria-hidden="true" className="shrink-0 text-[#657d68]" size={28} />
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {['Share needs', 'Match support', 'Plan next steps'].map((step, index) => (
                    <div key={step} className="flex items-center gap-3 rounded-2xl bg-[#f8f3ea] p-3">
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#eef3ec] text-sm font-black text-[#657d68]">
                        {index + 1}
                      </span>
                      <span className="text-sm font-bold text-[#26333b]">{step}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section id="services" className="bg-[#f8f3ea] py-20 md:py-28">
        <Container>
          <div className="mb-12 max-w-3xl">
            <p className="text-sm font-black uppercase text-[#657d68]">Services</p>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {services.map(({ title, text, icon }) => (
              <article key={title} className="rounded-[1.25rem] border border-[#e4d9c9] bg-[#fffaf1] p-6 shadow-sm shadow-[#2d4b58]/5">
                <ServiceIcon icon={icon} />
                <h3 className="mt-6 text-2xl font-black text-[#18242c]">{title}</h3>
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-2">
            {[
              ['Individual therapy intake', 'medical/mindwell/individual-therapy.png'],
              ['Couples counseling consultation', 'medical/mindwell/couples-counseling.png'],
            ].map(([label, image]) => (
              <div key={label} className="relative h-72 overflow-hidden rounded-[1.5rem] shadow-xl shadow-[#2d4b58]/8">
                ...
                <p className="absolute bottom-5 left-5 font-serif text-3xl leading-tight text-white">{label}</p>
              </div>
            ))}
          </div>
        </Container>
      ...
      <section id="intake" className="border-y border-[#e4d9c9] bg-[#efe7dc] py-20 md:py-28">
        <Container className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase text-[#657d68]">Intake process</p>
            <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-6xl">
              Therapy intake, made clear.
            </h2>
            <p className="mt-5 text-lg leading-8 text-[#5e635f]">
              The first step is organized around clarity: what you need, what appointment format fits, and what happens
              next.
            </p>
            <img
              src={imageUrl('medical/mindwell/intake-process.png')}
          ...
          <div className="rounded-[1.75rem] border border-[#d7cbbb] bg-[#fffaf1] p-4 shadow-xl shadow-[#2d4b58]/8">
            {intakeSteps.map(({ title, text }, index) => (
              <article key={title} className="grid gap-4 border-b border-[#e4d9c9] p-5 last:border-b-0 sm:grid-cols-[4rem_1fr]">
                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-[#2d4b58] text-sm font-black text-white">
                  {index + 1}
                </span>
                <div>
                  <h3 className="text-2xl font-black text-[#18242c]">{title}</h3>
                  <p className="mt-3 text-sm leading-7 text-[#5e635f]">{text}</p>
                </div>
              </article>
            ))}
      ...
      <section id="therapists" className="bg-[#fffaf1] py-20 md:py-28">
        <Container>
          <div className="mb-12 grid gap-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
            <div>
              <p className="text-sm font-black uppercase text-[#657d68]">Therapists</p>
              <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-6xl">
                Therapists who listen first.
              </h2>
            </div>
            <p className="max-w-3xl text-lg leading-8 text-[#5e635f]">
              Provider introductions are intentionally simple: who you may meet, what they focus on, and how the first
              conversation is framed.
            </p>
          </div>
          <div className="grid gap-5 lg:grid-cols-[1fr_1.05fr]">
            <img
              src={imageUrl('medical/mindwell/counseling-team.png')}
            ...
            <div className="grid gap-4">
              {providers.map(({ name, role, bio }) => (
                <article key={name} className="rounded-[1.25rem] border border-[#e4d9c9] bg-[#f8f3ea] p-6">
                  <h3 className="text-2xl font-black text-[#18242c]">{name}</h3>
                  <p className="mt-1 text-sm font-black text-[#657d68]">{role}</p>
      ...
      <section className="bg-[#f8f3ea] py-20 md:py-28">
        <Container className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <img
            src={imageUrl('medical/mindwell/calm-office.png')}
            alt="Calm therapy office seating"
            className="h-[35rem] w-full rounded-[1.75rem] object-cover shadow-xl shadow-[#2d4b58]/10"
          />
          <div>
            <p className="text-sm font-black uppercase text-[#657d68]">Approach</p>
            <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-5xl">
              A calm space for practical conversations.
            </h2>
            <p className="mt-5 text-lg leading-8 text-[#5e635f]">
              MindWell keeps counseling approachable through clear language, respectful pacing, and practical details
              before and after the first visit.
            </p>
            <div className="mt-8 grid gap-3">
              {approachPoints.map((point) => (
                <div key={point} className="flex items-center gap-3 rounded-2xl border border-[#e4d9c9] bg-[#fffaf1] p-4">
                  <CheckCircle aria-hidden="true" className="text-[#657d68]" size={20} />
      ...
      <section id="teletherapy" className="bg-[#2d4b58] py-20 text-white md:py-28">
        <Container className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase text-[#d7be7f]">Teletherapy</p>
            <h2 className="mt-3 font-serif text-4xl leading-tight md:text-6xl">
              Support that can meet you where you are.
            </h2>
            <p className="mt-5 text-lg leading-8 text-white/72">
              Virtual counseling appointments can be useful for ongoing sessions, busy schedules, and clients who want
              clear next steps before the first visit.
            </p>
          </div>
          <div className="relative overflow-hidden rounded-[1.75rem] bg-white/10 p-4 ring-1 ring-white/14">
            <img
              src={imageUrl('medical/mindwell/teletherapy.png')}
              alt="Virtual counseling appointment setup"
              className="h-80 w-full rounded-[1.25rem] object-cover"
            />
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {['Virtual counseling appointments', 'Good for ongoing sessions', 'Helpful for busy schedules', 'Private and convenient access'].map((item) => (
      ...
      <section id="visit-info" className="bg-[#fffaf1] py-20 md:py-28">
        <Container>
          <div className="mb-12 max-w-3xl">
            <p className="text-sm font-black uppercase text-[#657d68]">Visit info</p>
            <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-5xl">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {visitDetails.map(([title, text]) => (
              <article key={title} className="rounded-[1.25rem] border border-[#e4d9c9] bg-[#f8f3ea] p-6">
                <FileText aria-hidden="true" className="text-[#657d68]" size={22} />
                <h3 className="mt-5 text-xl font-black text-[#18242c]">{title}</h3>
                <p className="mt-3 text-sm leading-7 text-[#5e635f]">{text}</p>
              </article>
            ))}
          </div>
      ...
      <section className="bg-[#efe7dc] py-20 md:py-28">
        <Container className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase text-[#657d68]">Appointment access</p>
            <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-5xl">
              Begin with the right first conversation.
            </h2>
            <img
              src={imageUrl('medical/mindwell/appointment-access.png')}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {appointmentTypes.map((type) => (
              <div key={type} className="flex items-center gap-3 rounded-2xl border border-[#d7cbbb] bg-[#fffaf1] p-4">
                <Calendar aria-hidden="true" className="text-[#9b7d42]" size={20} />
      ...
      <section id="reviews" className="bg-[#f8f3ea] py-20 md:py-28">
        <Container>
          <div className="mb-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <p className="text-sm font-black uppercase text-[#657d68]">Reviews</p>
              <h2 className="mt-3 font-serif text-4xl leading-tight text-[#18242c] md:text-5xl">
                Clients notice when the first step feels understandable.
              </h2>
            </div>
            <img
              src={imageUrl('medical/mindwell/therapy-session.png')}
              alt="Comfortable therapy conversation"
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {reviews.map(({ name, quote }) => (
              <blockquote key={name} className="rounded-[1.5rem] border border-[#e4d9c9] bg-[#fffaf1] p-7">
                <p className="font-serif text-5xl leading-none text-[#9b7d42]">"</p>
                <p className="mt-2 text-lg font-bold leading-8 text-[#26333b]">{quote}</p>
                <footer className="mt-6 text-sm font-black uppercase text-[#657d68]">{name}</footer>
              </blockquote>
            ))}
          </div>
        </Container>
      ...
      <section id="contact" className="relative isolate overflow-hidden bg-[#18242c] py-20 text-white md:py-28">
        <img
          src={imageUrl('medical/mindwell/cta.png')}
          alt=""
          aria-hidden="true"
        ...
        <Container className="max-w-5xl">
          <p className="text-sm font-black uppercase text-[#d7be7f]">Start here</p>
          <h2 className="mt-3 font-serif text-4xl leading-tight md:text-6xl">
            Ready to take the first step?
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-white/74">
            Start with a simple intake request and get clear guidance for choosing the right support, appointment type,
            and next step.
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <CTAButton href="mailto:hello@mindwell.example" size="lg" className="rounded-full bg-[#d7be7f] text-[#18242c] hover:bg-white">
              Start Intake
            </CTAButton>
            <CTAButton
              href="#services"
          ...
          <div className="grid gap-8 md:grid-cols-[1.1fr_0.9fr_1fr]">
            <div>
              ...
              <p className="mt-3 max-w-md leading-7 text-white/66">
                Approachable therapy intake with clear next steps and supportive guidance.
              </p>
              <p className="mt-5 text-sm leading-6 text-white/58">
                If you are in immediate danger or experiencing a crisis, contact local emergency services or a crisis
            ...
            <div className="grid gap-3 text-sm text-white/70">
              <p className="flex gap-3">
                <MapPin aria-hidden="true" className="mt-0.5 shrink-0 text-[#d7be7f]" size={18} />
                <span>320 Willow Avenue, Suite 220</span>
              </p>
              <p className="flex gap-3">
                <Clock aria-hidden="true" className="mt-0.5 shrink-0 text-[#d7be7f]" size={18} />
                <span>Monday-Friday, 9 AM-7 PM</span>
              </p>
              <p className="flex gap-3">
                <Calendar aria-hidden="true" className="mt-0.5 shrink-0 text-[#d7be7f]" size={18} />
                <span>Saturday, 10 AM-2 PM</span>
              </p>
              <p className="flex gap-3">
                <Phone aria-hidden="true" className="mt-0.5 shrink-0 text-[#d7be7f]" size={18} />
                <a href="tel:5550136824" className="hover:text-white">(555) 013-6824</a>
              </p>
            </div>
          </div>
        </Container>
