  ...
  return (
    <a href="#home" className={`flex items-center gap-3 ${light ? 'text-[#F4F6F8]' : 'text-[#101722]'}`}>
      ...
      <span>
        ...
        <span className={`mt-1.5 block text-[0.58rem] font-semibold uppercase tracking-[0.36em] ${light ? 'text-[#F4F6F8]' : 'text-[#1C2736]'}`}>Realty</span>
      </span>
    </a>
  )
}
...
function SectionTitle({ label, title, body, light = false, centered = false }: { label: string; title: string; body?: string; light?: boolean; centered?: boolean }) {
  return (
    <div className={centered ? 'mx-auto max-w-4xl text-center' : 'max-w-3xl'} data-apex-reveal>
      <div className={`mb-5 flex items-center gap-3 text-[0.68rem] font-bold uppercase tracking-[0.28em] ${centered ? 'justify-center' : ''} text-[#7A2948]`}>
        <span className="h-px w-9 bg-[#7A2948]" />{label}
      </div>
      <h2 className={`apex-serif text-[clamp(2.6rem,5vw,4.9rem)] leading-[0.98] tracking-[-0.035em] ${light ? 'text-[#F4F6F8]' : 'text-[#101722]'}`}>{title}</h2>
      {body && <p className={`mt-6 max-w-2xl text-base leading-8 md:text-lg ${centered ? 'mx-auto' : ''} ${light ? 'text-[#F4F6F8]' : 'text-[#1C2736]'}`}>{body}</p>}
    </div>
  )
}
...
export function ApexCommercialRealty() {
  ...
  return (
    <main className="apex-site overflow-hidden bg-[#F4F6F8] text-[#1C2736]">
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled || menuOpen ? 'border-b border-black/10 bg-[#F4F6F8]/95 shadow-[0_10px_30px_rgba(16,23,34,0.08)] backdrop-blur-xl' : 'bg-[#F4F6F8]/90 backdrop-blur-md'}`}>
        <div className="mx-auto flex h-20 max-w-[92rem] items-center justify-between px-5 sm:px-8 lg:px-12">
          ...
          <nav className="hidden items-center gap-8 lg:flex" aria-label="Apex navigation">
            {navLinks.map((link) => <a key={link.label} href={link.href} className="text-[0.72rem] font-bold uppercase tracking-[0.12em] text-[#1C2736] transition hover:text-[#7A2948]">{link.label}</a>)}
          </nav>
      ...
      <section id="home" className="relative min-h-[850px] pt-20 lg:min-h-[930px]">
        <div className="absolute inset-0 top-20">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(16,23,34,0.95)_0%,rgba(16,23,34,0.78)_44%,rgba(16,23,34,0.18)_80%)]" />
        </div>
        <div className="relative mx-auto flex min-h-[770px] max-w-[92rem] items-center px-5 py-20 sm:px-8 lg:min-h-[850px] lg:px-12">
          <div className="max-w-4xl text-[#F4F6F8]">
            <p className="mb-7 flex items-center gap-4 text-[0.68rem] font-bold uppercase tracking-[0.3em] text-[#7A2948]"><span className="h-px w-12 bg-[#7A2948]" />Commercial advisory</p>
            <h1 className="apex-serif max-w-4xl text-[clamp(3.7rem,7.2vw,7.3rem)] leading-[0.9] tracking-[-0.045em]">Commercial Real Estate Built Around <em className="font-normal text-[#F4F6F8]">Market Intelligence</em></h1>
          ...
          <div className="absolute bottom-0 right-0 hidden bg-[#1C2736]/95 lg:grid lg:grid-cols-3">
            {[['$1.2B+', 'Transaction Volume'], ['340+', 'Properties Advised'], ['28', 'Active Markets']].map(([value, label]) => (
              <div key={label} className="min-w-[180px] border-l border-white/10 px-7 py-6"><p className="apex-serif text-3xl text-[#F4F6F8]">{value}</p><p className="mt-2 text-[0.58rem] font-bold uppercase tracking-[0.2em] text-[#F4F6F8]/60">{label}</p></div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[88rem]">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {categories.map((category, index) => (
              <article key={category.title} className="group border border-[#1C2736]/15 bg-[#F4F6F8] p-3 transition duration-300 hover:-translate-y-1 hover:border-[#7A2948]" data-apex-reveal style={{ transitionDelay: `${index * 80}ms` }}>
                ...
                <div className="p-5 pb-7">
                  ...
                  <h3 className="apex-serif mt-5 text-3xl text-[#101722]">{category.title}</h3>
                  <p className="mt-4 text-sm leading-7 text-[#1C2736]/70">{category.text}</p>
                  <a href="#contact" className="mt-7 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[#7A2948]">Discuss requirements <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></a>
                </div>
              </article>
            ))}
          </div>
      ...
      <section id="properties" className="bg-[#1C2736]/[0.06] px-5 py-24 sm:px-8 md:py-32 lg:px-12">
        <div className="mx-auto max-w-[92rem]">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {opportunities.map((property, index) => (
              <article key={property.title} className="group bg-[#F4F6F8] shadow-[0_18px_50px_rgba(16,23,34,0.08)] transition duration-300 hover:-translate-y-1" data-apex-reveal style={{ transitionDelay: `${index * 80}ms` }}>
                ...
                <div className="p-7">
                  <p className="flex items-center gap-2 text-[0.65rem] font-bold uppercase tracking-[0.14em] text-[#7A2948]"><MapPin className="h-3.5 w-3.5" />{property.location}</p>
                  <h3 className="apex-serif mt-4 text-[1.9rem] leading-tight text-[#101722]">{property.title}</h3>
                  <div className="mt-6 grid grid-cols-2 border-y border-[#1C2736]/15 py-4 text-xs">
                    <div><span className="block text-[0.55rem] font-bold uppercase tracking-[0.16em] text-[#1C2736]/55">Type</span><span className="mt-2 block font-semibold">{property.type}</span></div>
                    <div className="border-l border-[#1C2736]/15 pl-5"><span className="block text-[0.55rem] font-bold uppercase tracking-[0.16em] text-[#1C2736]/55">Size</span><span className="mt-2 block font-semibold">{property.size}</span></div>
                  </div>
                  <a href="#contact" className="mt-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[#101722]">View Details <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></a>
                </div>
              </article>
            ))}
          </div>
            ...
            <div className="max-w-xl">
              ...
              <div className="mt-10 grid gap-1">
                {['Local market demand analysis', 'Comparable property performance', 'Leasing and investment strategy'].map((item, index) => (
                  <div key={item} className="flex items-center gap-5 border-t border-white/12 py-5"><span className="grid h-9 w-9 place-items-center bg-[#7A2948]/25 text-[#F4F6F8]"><Check className="h-4 w-4" /></span><span className="text-sm font-semibold">{item}</span><span className="ml-auto text-xs text-[#7A2948]">0{index + 1}</span></div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-[88rem]">
          ...
          <div className="mt-14 grid border-l border-t border-[#1C2736]/15 sm:grid-cols-2 lg:grid-cols-3">
            {services.map(({ icon: Icon, title, text }, index) => (
              <article key={title} className="group border-b border-r border-[#1C2736]/15 p-7 transition hover:bg-[#1C2736]/[0.06] sm:p-9" data-apex-reveal style={{ transitionDelay: `${(index % 3) * 70}ms` }}>
                ...
                <h3 className="apex-serif mt-7 text-2xl text-[#101722]">{title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#1C2736]/70">{text}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[88rem]">
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {values.map((value, index) => (
              <article key={value.title} className="group min-h-[270px] border border-[#1C2736]/15 bg-[#F4F6F8] p-7 transition hover:border-[#7A2948] hover:shadow-[0_18px_40px_rgba(16,23,34,0.08)]" data-apex-reveal style={{ transitionDelay: `${index * 70}ms` }}>
                <span className="apex-serif text-4xl text-[#7A2948]">{value.number}</span>
                <h3 className="apex-serif mt-14 text-2xl text-[#101722]">{value.title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#1C2736]/70">{value.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="markets" className="px-5 py-24 sm:px-8 md:py-32 lg:px-12">
        <div className="mx-auto max-w-[92rem]">
          <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-end">
            ...
            <p className="max-w-2xl text-base leading-8 text-[#1C2736]/70 lg:justify-self-end">We advise on waterfront mixed-use assets, boutique hospitality spaces, adaptive retail properties, and commercial opportunities positioned in premium coastal markets.</p>
          </div>
            ...
            {coastalAssets.map((asset, index) => (
              <article key={asset.title} className={`group relative overflow-hidden ${index === 1 ? 'md:mt-12' : ''}`} data-apex-reveal>
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#101722] via-[#101722]/80 to-transparent p-7 pt-24 text-white">
                  <p className="text-[0.58rem] font-bold uppercase tracking-[0.22em] text-[#7A2948]">{asset.label}</p>
      ...
      <section id="contact" className="relative px-5 py-28 text-[#F4F6F8] sm:px-8 md:py-40 lg:px-12">
        <div className="absolute inset-0"><img src={ctaImage} alt="Coastal commercial building at dusk" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(16,23,34,0.96),rgba(16,23,34,0.78),rgba(16,23,34,0.48))]" /></div>
        <div className="relative mx-auto max-w-[88rem]" data-apex-reveal>
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.28em] text-[#7A2948]">Begin a conversation</p>
      ...
      <footer className="bg-[#101722] px-5 py-14 text-[#F4F6F8] sm:px-8 lg:px-12">
        <div className="mx-auto max-w-[88rem]">
          <div className="grid gap-12 border-b border-white/10 pb-12 md:grid-cols-[1.2fr_0.7fr_0.8fr_1fr]">
            <div><ApexMark light /><p className="mt-6 max-w-sm text-sm leading-7 text-[#F4F6F8]/60">Commercial property brokerage and investment advisory built on market intelligence, disciplined execution, and lasting value.</p></div>
