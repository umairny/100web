          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {comingSoonConcepts.map(([name, focus], index) => (
              <article key={name} className="group overflow-hidden border border-[#cfeaf6] bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="p-6"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#075985]">Software product</p><Link to={`/saas/${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="mt-3 inline-flex items-center rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-slate-600 transition hover:border-slate-400 hover:text-slate-900">Coming soon</Link><h3 className="mt-3 text-2xl font-black">{name}</h3><p className="mt-2 text-sm font-bold capitalize text-slate-500">{focus}</p><p className="mt-4 text-sm leading-6 text-slate-600">A conversion-focused SaaS concept explaining product value, core workflow, integrations, buyer trust, and a low-friction trial path.</p><div className="mt-5 flex gap-2 border-t border-slate-200 pt-5">{['#075985', '#f4fbff', '#14b8a6', '#082f49'].map((color) => <span key={color} className="h-6 w-6 rounded-full border border-slate-300" style={{ backgroundColor: color }} />)}</div><span className="mt-5 inline-flex w-full cursor-not-allowed items-center justify-center rounded-lg bg-slate-100 px-4 py-3 text-sm font-bold text-slate-500">Concept preview</span></div>
              </article>
            ))}
          </div>
        </Container>
